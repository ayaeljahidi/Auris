...
# FRONTEND_DIR becomes '.../Auris/frontend/public'
FRONTEND_DIR = BASE_DIR.parent / "frontend" / "public"

# 2. Mount Static Files using the absolute path
app.mount(
    "/static",
...
@app.get("/test")
async def test_page():
    # Use the absolute path to point to your HTML file
    test_file = FRONTEND_DIR / "test_questions.html"
    
    if not test_file.exists():
        return {"error": f"File not found at {test_file}"}
        
    return FileResponse(str(test_file))


@app.post("/questions/generate", response_model=QuestionsOutput)
async def generate_from_transcript(body: TranscriptInput):
    if not body.text.strip():
        raise HTTPException(status_code=400, detail="Text cannot be empty.")
    if len(body.text.split()) < 10:
        raise HTTPException(status_code=400, detail="Text too short.")

    loop = asyncio.get_event_loop()
    try:
        questions = await asyncio.wait_for(
            loop.run_in_executor(None, partial(generate_questions, body.text)),
            timeout=300    # ← matches the requests timeout
        )
    except asyncio.TimeoutError:
        raise HTTPException(status_code=504, detail="Generation timed out. Try a shorter text.")
