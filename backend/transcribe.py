...
def _flan_consumer(
    ...
    """Flan-T5 consumer thread - corrects segments concurrently."""
    ...
    while True:
        ...
        if needs_correction:
            ...
            log.debug("Flan seg[%d] %dms — %s→ %s", idx, seg_ms, "✏" if changed else "✓", corrected[:60])
        else:
...
def transcribe_whisper_with_correction_and_emotion(
    ...
    """
    Four-way pipeline: Whisper + Flan-T5 + Emotion + QG (optional).
    """
    t0 = time.perf_counter()
